...
.PHONY: \
  colab-install \
  train-scene-lm-base train-scene-lm-adapters train-scene-lm-all eval-gate-scene-lm error-analysis-scene-lm \
...
# Longer: make train-all OUTPUT_DIR=... MAX_TURNS=200
# Auto-resumes incomplete runs. Use --no-resume to force fresh training.
OUTPUT_DIR ?=
MAX_TURNS ?=
...
train-scene-lm-base: colab-install
	...
	@echo "################################################################"
	cd living_tales/trainer && $(ENV) PYTHONPATH=. \
	  python3 tools/train_scene_lm.py base \
	    --output-dir $(or $(OUTPUT_DIR),outputs) \
	    $(if $(CHECKPOINT_EVERY),--checkpoint-every $(CHECKPOINT_EVERY)) \
	    $(if $(RESUME),--resume)
...
train-scene-lm-adapters: colab-install
	...
	@for case in $(V2_CASES); do \
	  echo ""; echo "── adapter: $$case ──"; \
	  cd living_tales/trainer && $(ENV) PYTHONPATH=. \
	    python3 tools/train_scene_lm.py adapter --case $$case \
	      --output-dir $(or $(OUTPUT_DIR),outputs) \
	      $(if $(CHECKPOINT_EVERY),--checkpoint-every $(CHECKPOINT_EVERY)) \
	      $(if $(EVAL_EVERY),--eval-every $(EVAL_EVERY)) \
	      $(if $(RESUME),--resume) && cd ../.. || exit 1; \
	done
...
eval-gate-scene-lm:
	...
	@for case in $(V2_CASES); do \
	  echo "── $$case ──"; \
	  cd living_tales/trainer && $(ENV) PYTHONPATH=. \
	    python3 tools/eval_scene_lm.py $$case \
	      --model-path $(or $(OUTPUT_DIR),outputs)/$$case/scene_lm_full.pt && cd ../.. || exit 1; \
	done

error-analysis-scene-lm:
	@echo ""
	@echo "================================================================"
	@echo "  ERROR ANALYSIS — SceneLM v3 (per-dim accuracy, confusions, outcome recall)"
	@echo "================================================================"
	@for case in $(V2_CASES); do \
	  echo "── $$case ──"; \
	  cd living_tales/trainer && $(ENV) PYTHONPATH=. \
	    python3 tools/error_analysis_scene_lm.py $$case \
	      --model-path $(or $(OUTPUT_DIR),outputs)/$$case/scene_lm_full.pt \
	      $(if $(OUTPUT_DIR),--report-dir $(OUTPUT_DIR)/$$case) && cd ../.. || exit 1; \
	done
